...
Tailwind CSS v4 is in use. Configuration lives in `app.css` — there is no
`tailwind.config.js`. Do not generate v3 config syntax.
...
Vitest is the test runner. Config lives in `vite.config.ts` alongside the
Vite config — no separate vitest config file needed. Tests live next to the
files they test (`lib/rules.test.ts` alongside `lib/rules.ts`).
...
- Server functions live in `app/functions/` — one file per operation
...
app/
  routes/
    ...
    session/$id.tsx        — workout execution (heart of the app)
